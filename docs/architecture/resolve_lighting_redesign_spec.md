...
| Option | Description | Pros | Cons |
|---|---|---|---|
| A (default) | **Constant ambient term** read from `scene_data_block.data.ambient_light_color_energy.rgb` (already plumbed via the existing scene_data uniform set). The RGB channels are *already* premultiplied by the ambient energy when the scene UBO is built (`render_scene_data_rd.cpp:172-182`), so use `.rgb` directly — do NOT multiply by `.w` again or you double-scale. The `.w` channel carries the original energy value for shader paths that need it separately; the resolve composition does not. Match the mesh shading paths' usage. | Simple, cheap, no new buffers; uses existing scene env consistently with mesh shading | Not physically motivated for outdoors HDR; requires artist tuning |
...
- **Ambient color source**: use `scene_data_block.data.ambient_light_color_energy.rgb` directly. RGB is already energy-premultiplied at UBO build time (`render_scene_data_rd.cpp:172-182`); multiplying by `.w` again would double-scale. Do NOT use `.w == 0` as a "missing environment" sentinel — `.w == 0` is a legitimate authored configuration (e.g., zero background energy) and the no-environment path explicitly seeds rgb to (1,1,1) and `.w` to 1 (`render_scene_data_rd.cpp:153-156`), not zero. Match what mesh shading paths in `forward_clustered/scene_forward_clustered.glsl` do — read RGB directly, no fallback. If a real "no env" fallback is ever needed, detect via scene environment validity, not via the field's value.
...
- [ ] **4. Resolve shader ambient injection** (Option 4.3A — no CPU plumbing): in `tile_resolve.glsl`, add `+ scene_data_block.data.ambient_light_color_energy.rgb * base_color * alpha` (or equivalent) in the composition step. Use `.rgb` directly — it is already premultiplied by the ambient energy at UBO build time (`render_scene_data_rd.cpp:172-182`); multiplying by `.w` would double-scale and disagree with mesh shading. No fallback gate on `.w` (which is a legitimate authored value, not a missing-env sentinel); match the mesh path's behavior of reading RGB directly. Commit.
