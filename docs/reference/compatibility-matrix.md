...
| --- | --- | --- | --- | --- |
| Windows | `editor-tested` | Conditional nightly Windows editor zips (godotgs-windows-x86_64-<tag>.zip) are published when the self-hosted Windows build succeeds for that release run. | SUPPORTED_PLATFORMS in modules/gaussian_splatting/config.py accepts windows.<br>Gaussian Production Gates builds a Windows editor and runs windows-vulkan runtime validation in .github/workflows/gaussian_production_gates.yml.<br>Fresh PR evidence: PR #368 Gaussian Production Gates run 26128105716 passed the blocking Windows streaming-gpu-ci runtime gate and uploaded runtime-validation-report.<br>Release Builds compiles the Windows editor but publishes it conditionally: publish_release runs when Linux succeeds, downloads Windows only if build_windows succeeded, and uses fail_on_unmatched_files: false.<br>The production evidence route in tests/ci/collect_production_evidence.ps1 exercises non-headless Vulkan runtime scripts on Windows. | Windows is the strongest in-repo validation lane, but the current fresh green evidence is from PR #368 rather than older failing master runs. Windows nightly assets are current public artifacts when that lane succeeds; they are not a workflow guarantee for every publish. No stable v* release is published yet. |
| Linux | `sample-project-tested` | Guaranteed nightly publish floor: Linux editor tarball in release runs where publishing succeeds. | SUPPORTED_PLATFORMS in modules/gaussian_splatting/config.py accepts linuxbsd.<br>Release Builds compiles and publishes the Linux editor in .github/workflows/release_builds.yml.<br>Baseline QA builds a Linux editor, imports the project, and runs baseline QA under xvfb in .github/workflows/baseline_qa.yml. | Linux remains the fastest public evaluation path and the guaranteed release asset floor. The documented validation includes a sample-project QA lane under xvfb, but it is still not an interactive editor or hardware-GPU validation row; no fresh run ID is added in this docs slice. |
...
| --- | --- | --- | --- | --- | --- | --- |
| Windows | `editor-tested` | Self-hosted Windows runner (OS version not yet published) | NVIDIA GeForce RTX 3090 | Vulkan 1.4.312 + Forward+ + safe render thread; NVIDIA driver version not yet published | .github/workflows/gaussian_production_gates.yml module-validation lane<br>tests/runtime/run_runtime_validation.py --gd-mode windows-vulkan<br>tests/ci/collect_production_evidence.ps1<br>PR #368 Actions run 26128105716 runtime-validation-report: passed=3/3; GPU Streaming Stress tier_1m within_budget=true, frame_p95_ms=159.624, residency_ratio=1.0, fallback_rate=0.0 | Fresh passing evidence is PR-scoped until the calibration lands on master. It proves the adapter, render route, and streaming runtime gate; Windows OS version and NVIDIA driver version are still not printed in repo-owned evidence. |
| Linux | `sample-project-tested` | Ubuntu 24.04.4 LTS (image ubuntu-24.04 20260323.65.1) | No discrete GPU exposed in the published artifact (xvfb / headless CPU-only lane) | mesa-vulkan-drivers 25.2.8-0ubuntu0.24.04.1 + xvfb headless import lane | .github/workflows/baseline_qa.yml CPU-only lane<br>.github/workflows/release_builds.yml Linux editor build lane | This row preserves the previously documented sample-project/headless evidence. It is not refreshed with a current run ID in this docs slice, and it is still a headless xvfb lane rather than an interactive editor or hardware-GPU validation row. |
