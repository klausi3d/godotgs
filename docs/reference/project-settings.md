...
> **Cluster-culling status (manual annotation, 2026-04-26).** Per-key state of the
> `rendering/gaussian_splatting/culling/*` settings:
>
> | Key | Status | Where it goes today |
> |---|---|---|
> | `cluster_culling_enabled` | **dormant** | Read by `modules/gaussian_splatting/interfaces/cluster_culler.cpp:104-105`, but `ClusterCuller` is never instantiated in production. |
> | `cluster_target_size` | **dormant** | Read by `modules/gaussian_splatting/interfaces/cluster_culler.cpp:107-108`, but `ClusterCuller` is never instantiated in production. |
> | `cluster_frustum_slack` | **dormant** | Registered at `modules/gaussian_splatting/core/gaussian_splat_manager.cpp:1035`; consumed **only** as pipeline-state hash input at `modules/gaussian_splatting/renderer/render_pipeline_stages.cpp:587`. **Not** read by `cluster_culler.cpp` (the table row below is misleading on this point until the next regeneration). |
...
>
> Note: `cluster_use_morton_order` and `cluster_use_indirect_dispatch` are **not** registered
> project settings — they are only struct fields on `GPUCuller::CullingConfig`
> (`modules/gaussian_splatting/interfaces/gpu_culler.h:56-57`) consumed as pipeline-state hash
> inputs (`render_pipeline_stages.cpp:588-589`). They do not appear in this file.
>
> `ClusterCuller` is registered (`modules/gaussian_splatting/register_types.cpp:142`) but the
> `Ref<ClusterCuller> cluster_culler` field at
> `modules/gaussian_splatting/interfaces/gpu_culler.h:318` is never assigned. Active coarse
> culling is the LOD/octree query in `GPUCuller::ensure_hierarchical_structure()` — see
> [../architecture/culling-and-hierarchy.md](../architecture/culling-and-hierarchy.md). Cleanup
> of the dormant keys is tracked under milestone #6, specifically issue
> [#167](https://github.com/klausi3D/godotGS/issues/167); do not duplicate that remediation
> here.
>
> The `Defined In` line numbers in the table below are stale because the auto-generator
> (`scripts/generate_project_settings_reference.py`) has not been re-run; current
> registrations are at `gaussian_splat_manager.cpp:1029` (`octree_max_depth`), `:1030`
> (`min_gaussians_per_leaf`), `:1033` (`cluster_culling_enabled`), `:1034`
> (`cluster_target_size`), `:1035` (`cluster_frustum_slack`), `:1042` (`opacity_aware_bounds`).
>
> This block is hand-maintained. The rest of this file is regenerated by the script above;
> preserve this annotation across regenerations until the dormant cluster stack is removed or
> revived per [../architecture/tier2_cluster_culling_spec.md](../architecture/tier2_cluster_culling_spec.md).
