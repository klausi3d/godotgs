...
void GaussianData::apply_animation_at_time(float p_time) {
    ...
    bool needs_position_cache = animate_positions && animated_positions_cache.size() != gaussian_count;
    bool needs_color_cache = animate_colors && animated_colors_cache.size() != gaussian_count;
    bool needs_opacity_cache = animate_opacities && animated_opacities_cache.size() != gaussian_count;
    ...
    if (animation_cache_dirty || p_time != last_animation_time || needs_position_cache || needs_color_cache || needs_opacity_cache) {
        if (animate_positions) {
            animated_positions_cache.resize(gaussian_count);
            animated_positions_valid_cache.resize(gaussian_count);
        ...
        if (animate_colors) {
            animated_colors_cache.resize(gaussian_count);
            animated_colors_valid_cache.resize(gaussian_count);
            for (uint32_t i = 0; i < gaussian_count; i++) {
                Color sampled_color;
        ...
        if (animate_opacities) {
            animated_opacities_cache.resize(gaussian_count);
            animated_opacities_valid_cache.resize(gaussian_count);
            for (uint32_t i = 0; i < gaussian_count; i++) {
    ...
    const bool can_apply_positions = animate_positions && animated_positions_cache.size() == gaussian_count && animated_positions_valid_cache.size() == gaussian_count;
    const bool can_apply_colors = animate_colors && animated_colors_cache.size() == gaussian_count && animated_colors_valid_cache.size() == gaussian_count;
    const bool can_apply_opacities = animate_opacities && animated_opacities_cache.size() == gaussian_count && animated_opacities_valid_cache.size() == gaussian_count;
    for (uint32_t i = 0; i < gaussian_count; i++) {
        // Check if there are animated values for this splat
        if (can_apply_positions && animated_positions_valid_cache[i]) {
            gaussians[i].position = animated_positions_cache[i];
        }

        if (can_apply_colors && animated_colors_valid_cache[i]) {
            gaussians[i].sh_dc = animated_colors_cache[i];
        }

        if (can_apply_opacities && animated_opacities_valid_cache[i]) {
