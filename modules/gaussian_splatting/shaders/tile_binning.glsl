...
uint gs_build_quantized_sh_metadata(uint encoded_total, bool dc_linear_rgb) {
    ...
    uint dc_bit = dc_linear_rgb ? SH_METADATA_DC_LINEAR_RGB : 0u;
    return first_count | (high_count << 8u) | (encoded_total << 16u) | (SH_ENCODING_RGB9E5 << 24u) | dc_bit;
}
...
void main() {
    ...
    bool chunk_dc_linear = (chunk_meta_buffer.chunk_meta[quant_id].flags & GS_CHUNK_FLAG_DC_LINEAR_RGB) != 0u;
    ...
    g.sh_dc = src.sh_dc;
    g.sh_metadata = gs_build_quantized_sh_metadata(6u, chunk_dc_linear);
    for (int i = 0; i < 12; ++i) {
        g.sh_encoded[i] = 0.0;
    }
    ...
    // Resolve-mode lighting expects the raster pass to carry the raw SH color and
    // applies indirect_sh_scale later in tile_resolve. Per-splat/both modes bake
    // the indirect scale here because no later SH rescale occurs.
