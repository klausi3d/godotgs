...
const QUEUE_PRESSURE_STARVATION_MIN_FRAMES := 40
const QUEUE_PRESSURE_STARVATION_FAIL_RATIO := 0.90
...
const SCHEDULER_CPU_P95_TO_FRAME_P95_FAIL_RATIO := 0.90
const QUEUE_PRESSURE_HIGH_MUTEX_WAIT_MIN_MS := 8.0
const QUEUE_PRESSURE_HIGH_MUTEX_WAIT_FAIL_MIN_FRAMES := 36
...
const CAMERA_PATH_POINTS := [
    Vector3(-180.0, 40.0, -180.0),
    Vector3(180.0, 40.0, -180.0),
    Vector3(180.0, 40.0, 180.0),
    Vector3(-180.0, 40.0, 180.0)
]

# Low budget + high max chunk cap to force budget-driven visible evictions.
const OVERRIDES := {
    "rendering/gaussian_splatting/streaming/vram_budget_mb": 64,
    "rendering/gaussian_splatting/streaming/vram_warning_threshold_percent": 70,
    "rendering/gaussian_splatting/streaming/min_chunks_in_vram": 4,
    "rendering/gaussian_splatting/streaming/max_chunks_in_vram": 96,
    "rendering/gaussian_splatting/streaming/max_chunk_loads_per_frame": 10,
    "rendering/gaussian_splatting/streaming/max_prefetch_loads_per_frame": 5,
    ...
    "rendering/gaussian_splatting/streaming/max_evictions_per_frame": 10,
...
func _record_skip(message: String, context: Dictionary = {}) -> void:
    if not context.is_empty():
        message = "%s | context=%s" % [message, str(context)]
    skip_reasons.append(message)
    print("%s %s" % [SKIP_MARKER, message])
...
func _build_dataset(size: int) -> GaussianData:
    ...
    for i in range(size):
        ...
        var sh_idx := i * 3
        sh_dc[sh_idx + 0] = rng.randf()
        sh_dc[sh_idx + 1] = rng.randf()
        sh_dc[sh_idx + 2] = rng.randf()
        opacities[i] = 0.85
...
func _run() -> void:
    ...
    for frame_idx in range(SAMPLE_FRAMES):
        ...
        if stream_state is Dictionary:
            ...
            max_sync_q = max(max_sync_q, _read_int(s, "scheduler_sync_fallback_queue_depth"))
    ...
    if max_scheduler_stall_frames >= SCHEDULER_STALL_FAIL_FRAMES or scheduler_stalled_diagnostic_frames > 0:
        _record_failure(
            "Scheduler forward progress stalled during eviction churn probe",
            {
    ...
    if failures.is_empty() and not strict_eval_ready:
        _record_skip(
            "Eviction churn probe inconclusive due to insufficient load/evict activity",
            {
    ...
    if strict_eval_ready and \
        ...
        _record_failure(
            "Queue pressure remained active with insufficient forward progress during eviction churn probe",
            {
                "queue_pressure_candidate_frames": queue_pressure_candidate_frames,
    ...
    if strict_eval_ready and \
        ...
        _record_failure(
            "Queue-pressure scan starvation correlated with p95 frame spikes during eviction churn probe",
            {
    ...
    if strict_eval_ready and \
            queue_pressure_active_frames >= QUEUE_PRESSURE_STARVATION_MIN_FRAMES and \
            frame_p95_ms >= FRAME_P95_SPIKE_MIN_MS and \
        ...
        _record_failure(
            "Scheduler CPU time dominated eviction-churn p95 frame spikes under queue pressure",
            {
                "queue_pressure_active_frames": queue_pressure_active_frames,
                "frame_avg_ms": frame_avg_ms,
                "frame_p95_ms": frame_p95_ms,
    ...
    if strict_eval_ready and \
        ...
        _record_failure(
            "Persistent queue pressure had high pack mutex wait with no upload progress during eviction churn probe",
            {
                "queue_pressure_candidate_frames": queue_pressure_candidate_frames,
                "queue_pressure_no_progress_frames": queue_pressure_no_progress_frames,
                "queue_pressure_no_progress_ratio": queue_pressure_no_progress_ratio,
    ...
    if not failures.is_empty():
        status = "failed"
    elif not skip_reasons.is_empty():
        status = "skipped"
    ...
    var payload := {
        ...
        "visible_eviction_ratio": 0.0 if sum_evicted == 0 else float(sum_visible_evicted) / float(sum_evicted),
        ...
        "max_sync_fallback_queue_depth": max_sync_q,
